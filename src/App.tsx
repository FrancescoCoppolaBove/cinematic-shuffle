...
export default function App() {
  ...
  const getPersonalRating = useCallback((id: number) => watchedMovies.find((m) => m.id === id)?.personal_rating ?? null, [watchedMovies]);

  const likedIds = useMemo(() => new Set(watchedMovies.filter((m) => m.liked).map((m) => m.id)), [watchedMovies]);

  // ─── Open movie detail (fullscreen) ──────────────────────────
  const openMovieDetail = useCallback(
    async (id: number, mediaType: 'movie' | 'tv', fromLabel?: string, playlist?: PlaylistItem[], playlistIndex?: number) => {
  ...
  const openRelatedMovie = useCallback(
  ...
  const preloadAdjacent = useCallback(
    async (playlist: PlaylistItem[], currentIndex: number) => {
      const toLoad = [currentIndex - 1, currentIndex + 1].filter((i) => i >= 0 && i < playlist.length);
  ...
  const handleSwipeToIndex = useCallback(
  ...
  const openWithPlaylist = useCallback(
    (id: number, mt: 'movie' | 'tv', playlist?: PlaylistItem[], index?: number, label?: string) => {
  ...
  const backLabel =
    navStack.stack.length > 1 ? navStack.stack[navStack.stack.length - 2].fromLabel : (navStack.current?.fromLabel ?? 'Indietro');
  ...
  return (
    <div className='flex flex-col bg-film-black text-film-text overflow-hidden' style={{ height: 'var(--app-height)' }}>
      <div className='fixed inset-0 pointer-events-none opacity-30 bg-grain z-50' />

      {/* PWA update banner */}
      {showUpdate && (
        <div className='fixed left-4 right-4 z-[70] animate-slide-up' style={{ top: '1rem' }}>
          <div className='bg-film-surface border border-film-accent/40 rounded-2xl px-4 py-3 flex items-center gap-4 shadow-2xl'>
            ...
            <div className='flex items-center gap-2 shrink-0'>
              <button onClick={dismissUpdate} className='text-film-subtle text-xs px-2 py-1'>
                Dopo
              </button>
              <button
                onClick={applyUpdate}
      ...
      {/* Toasts */}
      <div className='fixed right-4 z-[60] flex flex-col gap-2 pointer-events-none' style={{ top: '1rem' }}>
        {toasts.map((t) => (
          <div
            key={t.id}
            className={cn(
      ...
      <header
        ref={headerRef}
        className='shrink-0 z-40 border-b border-film-border bg-film-black/95 backdrop-blur-md'
      ...
      <main
        className={cn(
          ...
          view === 'shuffle' || view === 'search' || view === 'profile' ? 'overflow-hidden' : 'overflow-y-auto px-4 py-4 pb-6',
        )}
      ...
      >
        ...
        {view === 'home' && (
          <HomeView
            ...
            onAddToWatchlist={addToWatchlist}
            onRemoveFromWatchlist={removeFromWatchlist}
            onOpenMovieGlobal={(id: number, mt: 'movie' | 'tv', playlist?: PlaylistItem[], index?: number) =>
              openWithPlaylist(id, mt, playlist, index, 'Home')
            }
          />
        )}
        {view === 'shuffle' && (
          <ShuffleView
            ...
            watchedMovies={watchedMovies}
            onOpenMovieGlobal={(id: number, mt: 'movie' | 'tv', playlist?: PlaylistItem[], index?: number) =>
              openWithPlaylist(id, mt, playlist, index, 'Shuffle')
            }
          />
        )}
        {view === 'search' && (
          <SearchView
            {...sharedProps}
            onOpenMovieGlobal={(id: number, mt: 'movie' | 'tv', playlist?: PlaylistItem[], index?: number) =>
              openWithPlaylist(id, mt, playlist, index, 'Cerca')
            }
          />
        )}
              ...
              return (
                ...
                >
                  ...
                  {v === 'profile' ? (
                    <div
                      className={cn(
                        'w-5 h-5 rounded-full overflow-hidden border transition-colors',
