...
onOpenMovieGlobal,
  ...
  }, [onOpenMovieGlobal, loadingId, filtered]);

  const handleOpenRelated = useCallback((id: number, mediaType: 'movie' | 'tv') => {
    onOpenMovieGlobal?.(id, mediaType);
  }, [onOpenMovieGlobal]);
        ...
        <div className="text-center py-20 text-film-muted space-y-3">
          ...
          <div>
            ...
            <p className="text-xs mt-1 text-film-subtle">
              Add movies and shows with the "Watchlist" button
            </p>
          </div>
        </div>
      ) : (
