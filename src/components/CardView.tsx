...
export function CardView({
  ...
  async function getDetail(): Promise<TMDBMovieDetail | null> {
    try {
      return await getMovieDetail(item.id, item.media_type);
    } catch {
      return null;
    }
  }
  ...
  return (
    <div className='fixed inset-0 z-[85] bg-film-black flex flex-col' style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      {/* ── Header ── */}
      <div className='relative flex items-center px-5 pb-2 shrink-0' style={{ paddingTop: 'calc(env(safe-area-inset-top) + 12px)' }}>
        {/* Counter left */}
      ...
      >
        <div className='w-full h-full' style={{ transform: dragX ? `translateX(${dragX}px)` : 'none', transition }}>
          {poster ? (
            <img src={poster} alt={title} className='w-full h-full object-contain rounded-2xl' onError={() => setImgErr(true)} />
          ) : (
      ...
      <div className='shrink-0 px-5 pb-2 pt-3 border-t border-film-border/50 bg-film-black'>
        {bottomMode === 'cta' ? (
          ...
          <>
            <div className='grid grid-cols-4 gap-3'>
              ...
              <CtaBtn
                ...
                icon={isOnWatchlist ? <BookmarkCheck size={22} strokeWidth={2} /> : <Bookmark size={22} strokeWidth={1.5} />}
            ...
            {onOpenFull && (
              <button
                onClick={() => {
                  onOpenFull?.(item.id, item.media_type);
                  onClose();
                }}
          ...
          <div className='py-2'>
            ...
            <InlineStarRating value={personalRating} onChange={handleRateChange} />
            <button
...
function InlineStarRating({ value, onChange }: { value: number | null; onChange: (r: number | null) => void }) {
  ...
  return (
    ...
    >
      {[1, 2, 3, 4, 5].map((star) => {
        ...
        return (
          <svg key={star} viewBox='0 0 24 24' className='flex-1' style={{ height: 48 }}>
            {half && (
              <defs>
...
function CtaBtn({
  ...
  return (
    ...
    >
      ...
      <span className={cn('text-xs font-medium leading-none', active ? 'text-white/90' : 'text-white/40')}>{label}</span>
    </button>
  );
}
